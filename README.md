...
Перед отладкой проекта, **необходимо прошить** плату МК AVR скомпилированным **.hex** файлом проекта, используя сторонние методы, например обёртку [AVRDUDESS](https://github.com/ZakKemble/AVRDUDESS) с настройками прошивки для большого количества плат **Arduino** по USB.
...
## Особенности
### Зависание Atmel Studio во время простоя
Пока PyScAT не подключён к Atmel Studio 7.0, среда будет **зависать**. Это происходит из-за постоянного поиска отладчика. Дело в том, что сам отладчик PyScAT в это время ещё **не подключён** к Atmel Studio. Его стоит подключать **только** после проведения всех настроек в PyScAT и Atmel Studio.
### Зависание Atmel Studio во время отладки
Из-за **медленной** скорости работы **USB** (и текущего алгоритма синхронизации), во время просмотра новых данных в **окне Memory**, будет происходить **зависание**. Оно вызвано тем, что Atmel Studio не хранит данные, что находятся вне поля зрения пользователя. Они запрашиваются динамически у отладчика. А из-за этого, что некоторые регистры МК AVR могут меняться даже во время простоя (регистры ввода-вывода), было принято решение не хранить их в буфере отладчика PyScAT, а запрашивать каждый раз заново.
Временное **решение проблемы**: включить отображение пользователю только 1 колонки ячеек памяти за раз

![Настройки отображения](README/Columns.png)
